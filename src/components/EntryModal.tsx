...
export default function EntryModal({
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // If phone number matches a past guest, we force save as a BRAND NEW reservation (generating a new targetId)
    const targetId = (guestToEdit && !isOldPhone) ? guestToEdit.id : `reservation_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
